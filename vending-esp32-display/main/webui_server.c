...
void start_rest_server(void) {
    ...
    config.max_uri_handlers = 8;   /* Config + Yappy settings endpoints */
    config.max_open_sockets = 7;   /* Default — no dashboard polling competing for sockets */
    ...
    ESP_LOGI(TAG, "REST server started with %d endpoints", 7);
}
