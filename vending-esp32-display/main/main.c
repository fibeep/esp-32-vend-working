...
/* WiFi STA connection tracking (shared with webui_server.c for dashboard) */
bool wifi_sta_connected = false;

/* Yappy kiosk mode globals (shared with mdb_handler.c and yappy_handler.c) */
bool session_timer_reset_todo   = false;   /* Yappy resets the 60s MDB session timer */
bool yappy_kiosk_mode           = false;   /* Set true in app_main from Kconfig      */
uint16_t yappy_item_price       = 0;       /* Current item price (set by VEND_REQ)   */
uint16_t yappy_item_number      = 0;       /* Current item number (set by VEND_REQ)  */
                               ...
                               int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT) {
        ...
        case WIFI_EVENT_STA_DISCONNECTED:
            ...
            if (wifi_retry_num++ < WIFI_MAX_RETRY) {
                ...
                /*
                 * Too many retries — enable DNS captive portal so the
                 * user can reconfigure WiFi.  SoftAP and REST server
                 * are already started in app_main (always-on for kiosk
                 * dashboard access).
                 */
                start_dns_server();
            }
    ...
    if (event_base == IP_EVENT) {
        ...
        case IP_EVENT_STA_GOT_IP:
            ...
            /*
             * Stop only the DNS captive portal (if running from a
             * previous AP fallback).  The REST server stays running
             * so the kiosk dashboard remains accessible at 192.168.4.1.
             */
            stop_dns_server();

            /*
             * CRITICAL FIX for APSTA mode:
             *
             * In APSTA mode, the SoftAP's DHCP server can interfere
             * with lwIP's global DNS server list, causing getaddrinfo()
             * to fail for outbound HTTPS calls (like the Yappy API).
             *
             * After the STA gets an IP via DHCP, explicitly copy the
             * STA interface's DNS server into lwIP's global DNS slot
             * so that all hostname lookups use the router's DNS.
             */
            {
                ...
                if (sta_netif) {
                    ...
                    if (esp_netif_get_dns_info(sta_netif, ESP_NETIF_DNS_MAIN, &dns_info) == ESP_OK) {
                        const ip_addr_t *dns_addr = (const ip_addr_t *)&dns_info.ip;
                        dns_setserver(0, dns_addr);
                        ESP_LOGI(TAG, "Global DNS set to STA DNS: " IPSTR,
...
void app_main(void)
{
    ...
    /*
     * Start SoftAP and REST server immediately so the kiosk tablet
     * can always reach the dashboard at http://192.168.4.1/dashboard,
     * regardless of whether STA WiFi connects later.
     */
    start_softap();
    start_rest_server();

    /* ---- NVS: Read provisioning data (passkey + subdomain) ---- */
    {
        nvs_handle_t handle;
    ...
    /* ---- Fallback: use Kconfig defaults if NVS has no provisioning data ---- */
    if (my_subdomain[0] == '\0' || my_passkey[0] == '\0') {
        strncpy(my_subdomain, CONFIG_VMFLOW_DEFAULT_SUBDOMAIN, sizeof(my_subdomain) - 1);
        my_subdomain[sizeof(my_subdomain) - 1] = '\0';
        strncpy(my_passkey, CONFIG_VMFLOW_DEFAULT_PASSKEY, sizeof(my_passkey) - 1);
        my_passkey[sizeof(my_passkey) - 1] = '\0';
        ESP_LOGW(TAG, "NVS has no provisioning data — using Kconfig defaults: subdomain=%s", my_subdomain);
    ...
    {
        char myhost[64];
        snprintf(myhost, sizeof(myhost), "%s.panamavendingmachines.com",
                 my_subdomain[0] ? my_subdomain : "0");
    ...
    /* ---- Yappy Payment Handler ---- */
    /*
     * Initialize the Yappy HTTPS client and load credentials from NVS.
     * Must be called after NVS and WiFi are initialized.
     */
    yappy_init();

    /*
     * Initialize the ILI9488 SPI TFT display and LVGL.
     * Sets up SPI bus, panel driver, LVGL screens.
     * Must be called after yappy_init() so state struct exists.
     */
    display_init();

    /*
     * Enable kiosk mode — the machine auto-enters a cashless session
     * on READER_ENABLE and auto-restarts on SESSION_COMPLETE.  VEND_REQUEST
     * triggers Yappy QR generation instead of BLE/MQTT approval.
     *
     * Controlled by Kconfig YAPPY_KIOSK_MODE (defaults to y).
     */
#ifdef CONFIG_YAPPY_KIOSK_MODE
    yappy_kiosk_mode = true;
#else
    yappy_kiosk_mode = true;   /* Default for yappy firmware variant */
#endif
     ...
     *
     * Yappy poll task: Core 0, priority 1, 6 KB stack
     *   Polls Yappy API for payment confirmation.
     *   Runs on Core 0 — sleeps 3s between polls, minimal CPU load.
     *
     * Display update task: Core 0, priority 2, 4 KB stack
     *   Polls yappy_get_state() every 200ms and updates LVGL screens.
     *   Higher priority than Yappy poll for responsive screen updates.
     */
    ...
    xTaskCreatePinnedToCore(vTaskYappyPoll, "YappyPoll", 6144, NULL, 1, NULL, 0);
    xTaskCreatePinnedToCore(vTaskDisplayUpdate, "DisplayUpdate", 4096, NULL, 2, NULL, 0);
